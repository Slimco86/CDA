...
class LD():
    def __init__(self, lamda, material, model='LD'):
        ...
        if self.model == 'Con':
            ...
            import re
            epsilon = np.zeros(len(omega_light), dtype=complex)
            with open('VO2 met.txt','r') as f:
                for i,wvl in enumerate( self.lamda):
                    for line in f:
                        line=re.split(r'\t+',line)
                        if line[0]==round(wvl*1e9,1):
                            epsilon[i]=line[1]+1j*line[2]
